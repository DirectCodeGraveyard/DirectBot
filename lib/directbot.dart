...
check_user(CommandEvent event) {
  if (_config['admins'].split(" ").contains(event.from) && authenticated.contains(event.from)) return true;
...
start([String nickname, String prefix]) {
  load_config().then((config) {
    _config = config;
    BotConfig botConf = new BotConfig(nickname: nickname == null ? config["nickname"] : nickname, username: config["username"], host: config["host"], port: config["port"]);
